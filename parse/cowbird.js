...
export function Grammar(grammar, sT, debug) {
  console.log('Grammar', { grammar, sT, debug });
  try {
    // If we have valid input then proceed to the construct phase.
    if(typeof grammar === 'object' && typeof sT === 'string') {
      this.sT = sT;
      this.aST = sT + '`';
      this.tokens = [];
      this.productions = [];
    ...
    // Invalid input, throw an error.
      ...
      throw new Error('Grammar(object, string, boolean)');
    }
  } catch(e) {
    displayError(e);
  }
...
Grammar.prototype = {
  ...
  __construct: function(grammar) {
    ...
    function buildProductions(lhs) {
      console.log('buildProductions', { lhs });
      if(grammar[lhs] !== void 0) {
        augmentedGrammar[lhs] = buildAlternates(lhs, grammar[lhs]);
      } else {
        console.error('buildProductions', lhs);
        throw new Error(`buildProductions(${lhs})`);
        throw [0x10, lhs];
      }
    }
    ...
    console.log('check', this.sT, this.aST);
    ...
    // configuration set.
    (function () {
      var i, configSet, j, config, h, next, nextString, nextIndex;
    ...
    (function () {
      var i, row, configSet, j, token, table, action, config, configStr, productionIndex, nextSet;
  ...
  parse: function(stream, scope) {
    ...
    function findLongest() {
      var key, successor, search, reduction, match, longest, whitespace;
    ...
    try {
      ...
      while(true) {
        ...
        // Throw the "choked attempting to parse: ____" error.
          ...
          throw new Error(`choked attempting to parse '${stream.substr(index, 40)}'`);
