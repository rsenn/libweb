...
export class ECMAScriptParser extends Parser {
  ...
  expectIdentifier(no_keyword = false, private_id = false) {
    ...
    if(token.type != 'nullLiteral')
      if(
        ...
        throw new Error(
          `Expecting <Identifier> but got <${token.type}> with value '${token.value}' at ${token.loc}`
        );
      }
  ...
  expectPunctuators(punctuators, ast) {
    ...
    if(Array.isArray(punctuators)) {
      if(punctuators.indexOf(token.value) < 0) {
        throw new Error(
          `${loc}: Expected: ${punctuators.join(' ')}    Actual: ${
            token.value
          }` /*, loc.file,loc.line*/
        );
      }
      ...
      throw new Error(
        `${loc}: Expected: ${punctuators} Actual: ${token.lexeme}` /*, loc.file,loc.line*/
      );
    }
  ...
  parsePrimaryExpression() {
    ...
    if(!is_async && this.matchKeywords('this')) {
      ...
      throw new Error(
        `${this.token.loc ?? this.position ?? this.lexer.loc}: Unexpected token '${this.token}'`
      );
    }
    if(rest_of) {
...
*/
    ...
    while(true) {
      ...
      if(this.matchPunctuators(['{', '['])) {
        ...
        if(rest) {
          ...
          if(element)
            if(
              property.name == (element instanceof AssignmentPattern ? element.left : element).name
            )
              shorthand = true;
  ...
  parseObject(isClass = false, args = []) {
    ...
    let ret = new ctor(
      ...[...args, ctor === ClassDeclaration ? this.addNode(ClassBody, properties) : properties]
    );
  ...
  parseForStatement(insideFunction) {
    ...
    if(isForInStatement)
      return new (operator == 'in' ? ForInStatement : ForOfStatement)(
        left,
        right,
        statement,
        async
...
*/
    ...
    //Parse If Statement
      ...
      if(!insideIteration && brk.label === undefined)
        throw new Error(
          `break; statement can only be inside an iteration or with a label`,
          this.position()
        );
      stmt = brk;
...
*/
  ...
  parseProgram() {
    ...
    if(tokens[0] && tokens[0].type !== 'eof') {
      throw new Error(
        `Didn't consume all tokens: ${tokens[0].loc ?? inspect(tokens[0], inspectOptions)}`
      );
    }
  ...
  let esfactory = function(...args) {
    ...
    function Debug(position, end) {
      ...
      if(end) {
        ...
        if(lexed.length)
          annotate.push(
            `lexed[${lexed.map(t => Util.abbreviate(quoteStr(t.value), 40)).join(', ')}]`
          );
