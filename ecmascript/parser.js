...
Parser.prototype = {
  ...
  expectIdentifier(no_keyword = false) {
    ...
    if(
      !(
        ...
        (no_keyword && token.type == tokenTypes.keyword)
      )
    ) {
      throw new SyntaxError(
        `${this.position()} Expecting Identifier, but got ${
          token.type
  ...
  expectKeywords(keywords) {
    ...
    if(token.type !== tokenTypes.keyword) {
      throw new SyntaxError(
        `${this.position()} ${this.position()} Expecting Keyword, but got ${
          token.type
        } with value: ${token.value}`
      );
    }
    if(Array.isArray(keywords)) {
      if(keywords.indexOf(token.value) < 0) {
        throw new SyntaxError(
          `${this.position()} Expected: ${keywords}    Actual: ${token.value ||
            token.type}`
        );
      }
  ...
  expectPunctuators(punctuators, ast) {
    ...
    if(token.type !== tokenTypes.punctuator) {
      throw new SyntaxError(
        `${this.position()} Expecting Punctuator, but got ${
          token.type
        } with value: ${token.value}`,
        ast
      );
    ...
    if(Array.isArray(punctuators)) {
      if(punctuators.indexOf(token.value) < 0) {
        throw new SyntaxError(
          `${this.position()} Expected: ${punctuators}    Actual: ${
            token.value
          }`,
          ast
        );
      ...
      throw new SyntaxError(
        `${this.position()} Expected: ${punctuators} Actual: ${token.VALUE}`,
        ast
      );
    }
    return token;
  ...
  expectLiteral() {
    ...
    if(!isLiteral(token)) {
      throw new SyntaxError(
        `${this.position()} Expecting Literal, but got ${
          token.type
        } with value: ${token.value}`
      );
    }
  ...
  parsePrimaryExpression() {
    ...
    if(!is_async && this.matchKeywords('this')) {
      ...
      this.expectPunctuators(')');
      if(this.matchPunctuators('=>'))
        expression = this.parseArrowFunction(expression, is_async);
...
*/
    ...
    if(!is_async && this.matchKeywords('new')) {
      ...
      const result = this.parseNewOrCallOrMemberExpression(
        couldBeNewExpression,
        false
      );
  ...
  parseUnaryExpression() {
    ...
    if(this.matchKeywords('await')) {
      ...
      const argument = this.parseUnaryExpression();
      return {
        ast: new estree.UnaryExpression(
          operatorToken.value,
          argument.ast,
          true
        ),
      ...
      if(operatorToken.value === '++' || operatorToken.value === '--') {
        ast = new estree.UpdateExpression(
          operatorToken.value,
          argument.ast,
          true
        );
  ...
  parseBinaryExpression(minPrecedence) {
    ...
    //  this.log('TOKEN: ', tok);
    while(
      (this.matchKeywords('instanceof') ||
        this.matchPunctuators(punctuators) ||
        tok.value == 'instanceof') &&
      ...
      const operatorToken =
        value == 'instanceof'
          ? this.expectKeywords('instanceof')
  ...
  parseExpression(optional) {
    ...
    if(expression !== null) {
      ...
      const token = this.next();
      throw new SyntaxError(
        `${this.position()} Expecting AssignmentExpression, but got ${
          token.type
        } with value: ${token.value}`
      );
    }
  ...
  parseBindingPattern() {
    ...
    return tok.value == '{'
      ? new estree.ObjectBinding(props)
      : new estree.ArrayBinding(props);
  },
  ...
  parseObject() {
    ...
    if(ctor == estree.ObjectBinding) {
      members = Object.entries(members).map(
        ([key, value]) =>
          new estree.BindingProperty(
            new estree.Identifier(key),
            value ? new estree.Identifier(value) : new estree.Identifier(key)
          )
      );
    }
...
*/ break;
  ...
  parseIfStatement(insideIteration, insideFunction) {
    ...
    if(this.matchKeywords('else')) {
      ...
      if(alternate === null) {
        throw new SyntaxError(
          'Expecting statement for else block in if-statement'
        );
      }
    }
    ...
    if(this.matchKeywords(['var', 'let', 'const'])) {
      ...
      if((keyw = this.matchKeywords(['in', 'of']))) {
        ...
        if(ast.declarations.length !== 1) {
          throw new SyntaxError(
            `${this.position()} Expecting only one Identifier and at most one Initializer in a ForIn statement`
          );
        }
  ...
  parseStatement(insideIteration, insideFunction) {
    ...
    // Parse With Statement
      ...
      if(insideIteration) {
        ...
        throw new SyntaxError(
          `${this.position()} continue; statement can only be inside an iteration`
        );
      }
      ...
      if(insideIteration) {
        ...
        throw new SyntaxError(
          `${this.position()} break; statement can only be inside an iteration`
        );
      }
      ...
      if(insideFunction) {
        ...
        throw new SyntaxError(
          `${this.position()} return statement can only be inside a function`
        );
      }
    } else {
  ...
  parseProgram() {
    ...
    if(this.tokens.length >= 1 && this.tokens[0].type !== tokenTypes.eof) {
      throw new SyntaxError(
        `Didn't consume all tokens: ${Util.inspect(this.tokens[0])}`
      );
    }
