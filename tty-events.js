...
class KeyboardEvent {
   ...
   */
  constructor({ name, sequence, isSpecial = false, ctrl = false, alt = false, shift = false }) {
...
class Terminal extends EventEmitter {
  ...
  constructor(
    ...
    this._dataListener = buf => {
      for(let i = 0; i < buf.length; i++) {
        if(sendByte) sendByte = iterator.next(buf[i]).value;
        else {
          // Decode a character
          ...
          if(str)
            for(let char of str) {
              if(iterator.next(char).value) {
                // Break when a byte is expected, discarding the rest of the string (this should never happen).
                sendByte = true;
                break;
              }
            }
  ...
  enableMouse(mode = 0, sgr = true) {
    ...
    if(mode === 1) this.output.write(`\x1b[?1001h`);
    else {
      if(mode >= 2) this.output.write(`\x1b[?1002h`);

      if(mode === 3) this.output.write(`\x1b[?1003h`);
    }

    if(sgr) this.output.write(`\x1b[?1006h`);
  }
  /**
...
function* emitKeys(term) {
  ...
  function parseAndEmitMouse(b, x, y, mouseUp = false) {
    if(b < 0 || x < 1 || y < 1 || isNaN(b) || isNaN(x) || isNaN(y))
      // Sometives rxvt sends mouse sequences with negative coordinates.
      return;
    ...
    if(b & 64 || b & 128) {
      if(b & 64) button |= 4;
      if(b & 128) button |= 8;
    } else if(button === 3) button = undefined;
    else button += 1; // Only increment button if in the range 0-2.
  ...
  main: while(true) {
    ...
    if(ch === '\x1b') {
      ...
      if(ch === '' || (s.length > 1 && ch === '\x1b')) {
        // ESC [ESC] timeout *or* ESC ESC ESC
        term.emit(
          'keypress',
          new KeyboardEvent({
            name: 'escape',
    ...
    if(escaped && (ch === 'O' || ch === '[')) {
      ...
      if(ch === 'O') {
        ...
        while(true) {
          ...
          if((charCode >= 0x40 && charCode <= 0x7e) || charCode === 0x24) {
            // Not in the spec, but $ can end a sequence.
            // End of sequence
            break;
        ...
        if(
          ...
          if(ch === 'M' || ch === 'm' || ch === 'T' || ch === 't') {
            ...
            while(true) {
              str += ch = yield;
              if(ch === endSeq[endSeqIndex]) endSeqIndex++;
              else endSeqIndex = 0;
