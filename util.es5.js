const formatAnnotatedObject = function (subject, {
  ...
  if (typeof subject == "object" && "map" in subject && typeof subject.map == "function") {
    return `[${nl}${subject.map(i => formatAnnotatedObject(i, opts)).join(separator + nl)}]`;
  }
  ...
  if (map.entries === undefined) {
    map.entries = function* iterator() {
      for (let entry of map) {
        if (entry.name !== undefined && entry.value !== undefined) yield [entry.name, entry.value];else if (entry[0] !== undefined && entry[1] !== undefined) yield entry;else yield [entry, map[entry]];
      }
    };
  }
...
Util.members = function (obj, recursive = false, pred = () => true) {
  ...
  return function* () {
    ...
    if (recursive) for (let proto of Util.getPrototypeChain(obj)) for (let name of Object.getOwnPropertyNames(proto)) if (adder(name)) yield name;
  }();
};
...
Util.getMethods = function (obj, recursive = false) {
  ...
  return names.reduce((ret, method) => ({ ...ret,
    [method]: obj[method]
  }), {});
};
...
Util.immutableClass = Original => {
  ...
  return new Function("Original", `const Immutable${name} = class extends Original {
    constructor(...args) {
      super(...args);
  ...
  return Immutable${name};`)(Original);
};
