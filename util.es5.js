...
var formatAnnotatedObject = function formatAnnotatedObject(subject, _ref) {
  ...
  var indent = _ref$indent === void 0 ? '  ' : _ref$indent;
  var _ref$spacing = _ref.spacing;
  var spacing = _ref$spacing === void 0 ? ' ' : _ref$spacing;
  var _ref$separator = _ref.separator;
  var separator = _ref$separator === void 0 ? ',' : _ref$separator;
  var _ref$newline = _ref.newline;
  var newline = _ref$newline === void 0 ? '\n' : _ref$newline;
  var _ref$maxlen = _ref.maxlen;
  var maxlen = _ref$maxlen === void 0 ? 30 : _ref$maxlen;
  var _ref$depth = _ref.depth;
  var depth = _ref$depth === void 0 ? 1 : _ref$depth;
  ...
  for(var k in subject) {
    ...
    if(typeof subject[k] === 'symbol') {
      ...
      try {
        s = depth <= 0 ? 'Array('.concat(subject[k].length, ')') : '[ ' + _toConsumableArray(subject[k]).join(',') + ' ]';
      } catch(err) {
        s = '[' + subject[k] + ']';
      }
...
Util.draw = function(arr, n) {
  var r = Util.shuffle(arr).splice(0, n); //console.log("Util.draw ", { arr, n, r });
...
Util.extendMap = function(map) {
  if(map.entries === undefined) {
    map.entries =
      /*#__PURE__*/
      _regeneratorRuntime.mark(function iterator() {
  ...
  if(typeof value == 'function') pred = value;
    ...
    pred = function pred(obj) {
      for(var j = 0; j < prop.length; j++) {
        if(obj[prop[j]] == value) return true;
      }
...
Util.removeEqual = function(a, b) {
  ...
  for(var key in _objectSpread({}, a)) {
...
Util.clearCookies = function(c) {
  return Util.setCookies(
    _Object$keys(Util.parseCookie(c)).reduce(function(acc, name) {
      return _objectSpread({}, acc, _defineProperty({}, name, '; max-age=0; expires=' + new Date().toUTCString()));
    }, {})
  );
...
Util.parseURL = function() {
  ...
  var argstr = matches[4].replace(/^[^?]*\?/, '?') + '&test=1';
  var pmatches = typeof argstr === 'string' ? /[?&]([^?&=]*)=([^?&]*)/g.exec(argstr) : Util.array();
...
Util.mergeLists = function(arr1, arr2) {
  ...
  var hash = arr1.reduce(function(acc, it) {
    return _objectSpread(_defineProperty({}, it[key], it), acc);
  }, {});
  ...
  return Util.mergeObjects([
    ...
    _toConsumableArray(document.forms).reduce(function(acc, form) {
      return _toConsumableArray(form.elements).reduce(function(acc2, e) {
        return e.name == '' || e.value == undefined || e.value == 'undefined' ? acc2 : _objectSpread({}, acc2, _defineProperty({}, e.name, e.value));
      }, acc);
    }, {})
  ]);
};
...
Util.getMethodNames = function(obj) {
  return Util.array(
    Util.members(obj).filter(function(item) {
...
Util.getMethods = function(obj) {
  ...
  return names.reduce(function(ret, method) {
    return _objectSpread({}, ret, _defineProperty({}, method, obj[method]));
  }, {});
};
