...
Util.decamelize = (str, separator = "-") => /[A-Z]/.test(str) ? str.replace(/([a-z\d])([A-Z])/g, "$1".concat(separator, "$2")).replace(/([A-Z]+)([A-Z][a-z\d]+)/g, "$1".concat(separator, "$2")).toLowerCase() : str;
...
Util.getMethods = function (obj) {
  ...
  return names.reduce((ret, method) => Object.assign(ret, {
    [method]: obj[method]
  }), {});
};
