...
var formatAnnotatedObject = function formatAnnotatedObject(subject, _ref) {
  ...
  if (typeof subject == "object" && "map" in subject && typeof subject.map == "function") {
    return "[" + nl + subject.map(function (i) {
...
Util.draw = function (arr, n) {
  ...
  var r = Util.shuffle(arr, rnd).splice(0, n);
  return r;
};
...
Util.randomNumbers = function (_ref2, draws) {
  ...
  var r = Util.sortNum(Util.draw(Util.range(start, end), draws));
  return r;
};
...
Util.define = function (obj, key, value) {
  ...
  return obj[key] === undefined && Object.defineProperty(obj, key, {
...
Util.adapter = function (obj) {
  ...
  var adapter = (_adapter = {
    ...
    keys: _regeneratorRuntime.mark(function keys() {
      var length, i;
    ...
    entries: _regeneratorRuntime.mark(function entries() {
...
Util.extendMap = function (map) {
  if (map.entries === undefined) {
    map.entries = _regeneratorRuntime.mark(function iterator() {
...
Util.encodeCookie = function (c) {
  return Object.entries(c).map(function (_ref4) {
...
Util.clearCookies = function (c) {
  return Util.setCookies(Object.keys(Util.parseCookie(c)).reduce(function (acc, name) {
...
Util.getURL = function () {
  ...
  var port = process.env.PORT ? parseInt(process.env.PORT) : process.env.NODE_ENV === "production" ? 443 : 8080;
...
Util.parseURL = function () {
  ...
  var argstr = matches[4].indexOf("?") != -1 ? matches[4].replace(/^[^?]*\?/, "") : "";
...
Util.makeURL = function () {
  var args = Array.prototype.slice.call(arguments);
  var href = typeof args[0] == "string" ? args.shift() : Util.getURL();
  var url = Util.parseURL(href);
  var obj = typeof args[0] == "object" ? args.shift() : {};
...
Util.formatRecord = function (obj) {
  ...
  for (var key in obj) {
    ...
    });else if (/^-?[0-9]+$/.test(val)) val = parseInt(val);else if (/^-?[.0-9]+$/.test(val)) val = parseFloat(val);else if (val == "true" || val == "false") val = Boolean(val);
    ret[key] = val;
  }
...
Util.walkTree = _regeneratorRuntime.mark(function _callee(node, pred, t) {
  var depth,
      parent,
      _i,
      _arr,
