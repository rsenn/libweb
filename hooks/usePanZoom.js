import { useRef, useState, useEffect, useCallback } from '../dom/preactComponent.js';
...
function usePanZoom({
  ...
  useEffect(() => {
    ...
    function onTouchStart(e) {
      e.preventDefault();
      const touches = getInsideTouch(elemRectRef.current, styleRef.current, e.touches);
      // pan
      if(touches.length === 1) {
    ...
    function onTouchEnd(e) {
      e.preventDefault();
      const touches = getInsideTouch(elemRectRef.current, styleRef.current, e.changedTouches);
      // pan
      if(touches.length === 1) {
