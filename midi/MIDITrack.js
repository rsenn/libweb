...
export class MIDITrack {
  ...
  readBytes(stream) {
    ...
    if(header != MTrk) throw new Error('Invalid track - Expected MTrk');
    ...
    while(bytes < chunkSize) {
      ...
      if(event instanceof MIDIMetaEvent) {
        if(event.type == MIDIMetaEvent.END_OF_TRACK) eot = true;
        else if(event.type == MIDIMetaEvent.TRACK_NAME) this.name = event.text;
      }

      bytes += stream.position - cursor;
    }

    if(!eot) throw new Error('Expected end of track event');

    if(bytes < chunkSize) throw new Error('Read less bytes than track chunk size');
  }
  ...
  writeBytes(stream) {
    ...
    for(var i = 0; i < this.events.length; i++) {
      if(this.events[i].type != MIDIMetaEvent.END_OF_TRACK) this.events[i].writeBytes(stream);
    }
  ...
  toString() {
    ...
    for(var i = 0; i < this.events.length; i++) string += this.events[i].toString() + '\r\n';
