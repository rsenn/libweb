...
export class Element extends Node {
  ...
  static toCommand(elem, parent = "") {
    ...
    s += `Element.create('${tagName}', { ${Object.entries(attrs).map(([k, v]) => `${k}: ${JSON.stringify(v)}`).join(", ")} }${parent ? `, ${parent}` : ""});`;
    let child;
  ...
  static rect(elem, options = {}) {
    ...
    if (options.border) {
      ...
      Rect.outset(r, border); // console.log("Element.rect(", r, ") // with border = ", border);
    }
  ...
  static setRect(element, rect, anchor = Anchor.LEFT | Anchor.TOP) {
    ...
    const trbl = Rect.toTRBL(rect); //console.log('Element.setRect ', { trbl, ptrbl, stack });
  ...
  static resize() {
    ...
    const css = Size.toCSS(size); //console.log("Element.resize: ", { e, size, css });
  ...
  static setCSS(element, prop, value) {
    ...
    for (let key in prop) {
      ...
      if (element.style) {
        if (element.style.setProperty) element.style.setProperty(propName, value);else element.style[Util.camelize(propName)] = value;
      }
    }
  ...
  static getCSS(element, property = undefined, receiver = null) {
    ...
    const pstyle = parent && parent.tagName ? Util.toHash(w && w.getComputedStyle ? w.getComputedStyle(parent) : d.getComputedStyle(parent)) : {}; //console.log('Element.getCSS ', { estyle, pstyle });
        ...
        if (typeof result == "object") {
          try {
            Object.defineProperty(result, "cssText", {
              get: function () {
                return Object.entries(this).map(([k, v]) => `${Util.decamelize(k, "-")}: ${v};\n`).join("");
              },
              enumerable: false
            });
          } catch (err) {}
  ...
  static transition(element, css, time, easing = "linear") {
    ...
    const tlist = a.join(", "); //console.log("Element.transition", ctx);
