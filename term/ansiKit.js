...
class AnsiKit {
   ...
   * If <code>color</code> is not provided, is not a number or is more than 255/less than 0, it will default to 0
   * @param  {number} color Color of the text
   * @return {AnsiKit}
   */
  static color256(color = 0) {
    if(color >= 256 || color <= -1 || typeof color !== 'number') color = 0;
   ...
   * @param  {number} style Style to use. If style > 6 or < 1, it will be the {@link AnsiKit#underlineCursor underline cursor} (3)
   * @return {AnsiKit}
   * @see    blockCursor
   */
  ...
  /**
   * Formats text, then prints it with a newline.
   * @param  {string} text The text to format
   * @return {AnsiKit}
   */
  static println(text = '') {
    return AnsiKit.print(`${text}\n`);
  }
  ...
  /**
   * Restores the cursor's most recently saved position.
   * @return {AnsiKit}
   */
  static restoreCursor() {
    return AnsiKit.printCSI(null, 'u');
  }
   ...
   */
  static saveCursor() {
    return AnsiKit.printCSI(null, 's');
  }
...
AnsiKit.blockCursor = 1;
/** A block cursor. */
AnsiKit.blockCursorSteady = 2;
/** A blinking underline cursor. */
AnsiKit.underlineCursor = 3;
/** A underline cursor. */
AnsiKit.underlineCursorSteady = 4;
/** A standard blinking line cursor. */
AnsiKit.lineCursor = 5;
/** A standard line cursor. */
AnsiKit.lineCursorSteady = 6;
