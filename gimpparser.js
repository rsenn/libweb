...
var stringParser = new Parser().string('data', { zeroTerminated: true });

var rgbParser = new Parser().uint8('red').uint8('greed').uint8('blue');
...
var parasiteParser = new Parser().uint32('length').buffer('parasite', { length: 'length' });
...
var hirerarchyParser = new Parser().uint32('width').uint32('height').uint32('bpp').uint32('lptr');
  ...
  copyTile(image, tileBuffer, xoffset, yoffset, xpoints, ypoints, bpp, mode) {
    ...
    for(var yloop = 0; yloop < ypoints; yloop += 1) {
      for(var xloop = 0; xloop < xpoints; xloop += 1) {
        ...
        var bgCol = image.getAt(xoffset + xloop, yoffset + yloop);
        if(mode) {
          colour = mode.compose(bgCol, colour);
        }
