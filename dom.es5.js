...
exports.default = exports.isSelect = exports.isRenderer = exports.isReactComponent = exports.isSVG = exports.isContainer = exports.isCSS = exports.isTree = exports.isTimer = exports.isTRBL = exports.isPointList = exports.RandomColor = exports.TransitionList = exports.Transition = exports.CSSTransformSetters = exports.ElementTransformation = exports.ElementRectProps = exports.ElementSizeProps = exports.ElementPosProps = exports.ElementWHProps = exports.ElementXYProps = exports.Select = exports.Renderer = exports.ReactComponent = exports.SVG = exports.Container = exports.CSS = exports.Element = exports.Node = exports.Timer = exports.isHSLA = exports.isRGBA = exports.isLine = exports.MatrixProps = exports.isMatrix = exports.isRect = exports.isSize = exports.isPoint = exports.Anchor = exports.isNumber = void 0;
...
function dom() {
  ...
  for (let e of args) {
    if (e instanceof SVGSVGElement) extend(e, SVG);else if (e instanceof HTMLElement) {
      extend(e, Element);
      ElementPosProps(e);
      ElementSizeProps(e);
    }
...
Point.fromAngle = (angle, dist = 1.0) => new Point({
...
Point.angle = (p1, p2 = {
  x: 0,
  y: 0
}) => Point.toAngle(Point.diff(p1, p2));
...
Point.toString = point => {
  ...
  return `${point.x},${point.y}`; //  return Point.prototype.toString.call(point);
};
...
function PointList(points) {
  ...
  if (typeof points === "string") {
    const matches = [...points.matchAll(/[-.0-9,]+/g)]; //console.log('points: ', points);
...
PointList.prototype.toPolar = function (tfn) {
  ...
  ret.splice.apply(ret, [0, ret.length, ...this.map(p => {
    const angle = Point.toAngle(p); // console.log("Angle: ", angle);
...
PointList.prototype.normalizeY = function (newVal = y => y) {
  ...
  const ydiff = yrange[1] - yrange[0]; //console.log('yrange ', yrange);
...
PointList.prototype.toString = function () {
  return "[" + this.map(point => Point.toString(point)
  /*.toString && !(point instanceof DOMPoint) ? point.toString() : Point.toString(point))*/
  ).join(",\n  ") + "]";
};
...
function Size(arg) {
  ...
  if (isNaN(obj.height)) obj.height = undefined; //console.log('Size', obj, ' arg=', arg);
...
Rect.extrema = () => {
  ...
  const f = args.shift();
  return args.reduce(function (ex, pt) {
    return {
...
Rect.resize = () => {
  let args = Array.from(arguments);
  let rect = void 0 == Rect ? args.shift() : void 0;
...
Rect.toSource = (r = void 0) => "{x:" + r.x + ",y:" + r.y + ",width:" + r.width + ",height:" + r.height + "}";

Rect.prototype.toSource = (r = void 0) => "new Rect(" + (r ? r.x + "," + r.y + "," + r.width + "," + r.height : "") + ")";

Rect.equal = (a, b) => a.x == b.x && a.y == b.y && a.width == b.width && a.height == b.height;

Rect.indexOf = (r, a, skip = () => false) => a.reduce((accu, item, i) => {
  return accu == -1 && !skip(item) && Rect.equal(r, item) ? i : accu;
}, -1);

Rect.lastIndexOf = (r, a, skip = () => false) => a.reduceRight((accu, item, i) => {
  return accu == -1 && !skip(item) && Rect.equal(r, item) ? i : accu;
}, -1);
...
_util.default.defineGetterSetter(Rect.prototype, "x1", function () {
  return this.x;
}, function (value) {
...
Rect.center = r => new Point(r.x + r.width / 2, r.y + r.height / 2); //Rect.prototype.center = function() { return new Point(this.x + this.width / 2, this.y + this.height / 2); }
...
Rect.transform = function (rect1, rect2) {
  ...
  let scale = {
    x: rect1.width / rect2.width,
    y: rect1.height / rect2.height
  };
  let matrix = Matrix.identity(); // matrix = Matrix.scale(matrix, scale.x, scale.y);
...
const isMatrix = m => m instanceof Matrix || m.length !== undefined && m.length == 6 && m.every(el => typeof el == "number");
...
const MatrixProps = Object.keys(Matrix.keyIndex).reduce((acc, k) => {
...
Matrix.scalar_multiply = (m, f) => Object.assign(m, {
  xx: m[0] * f,
  yx: m[1] * f,
  xy: m[2] * f,
  yy: m[3] * f,
...
Matrix.set = function (a) {
  return Matrix.init.apply(Matrix, [this, ...Array.from(a)]); //[ this.xx,this.yx, this.xy,this.yy, this.x0,this.y0 ] = a;
  //return this;
};
...
Matrix.t = function (op, ...args) {
  ...
  let m = fn.apply(Matrix, Array.from(args)); //console.log('transformation matrix: ' + this.toString());
...
Matrix.init = function () {
  ...
  const abcdef = "abcdef";
  Matrix.keys.forEach((key, i) => matrix[i] = args[Matrix.keySeq.indexOf(key)]); //matrix.splice.apply(matrix, [0, matrix.length, ...args]);
...
Matrix.scale = function (m, sx, sy) {
  let args = [...arguments]; //  if(typeof args[0] == "number") return Matrix.init_scale.call(new Matrix(), sx, sy);
  //  let m = args.shift();
...
Matrix.toString = m => {
  ...
  for (let i = 0; i < m.length; i++) {
    ...
    r += _util.default.pad(m[i].toString(), 3) + m[i];
  }
...
Matrix.point_transformer = m => function (arg) {
  var matrix = m;
...
["a", 0].forEach(prop => _util.default.defineGetterSetter(Line.prototype, prop, function () {
  let p = new Point(this.x1, this.y1);

  _util.default.defineGetterSetter(p, "x", () => {
    return this.x1;
  }, v => {
    this.x1 = v;
  }, true);

  _util.default.defineGetterSetter(p, "y", () => {
    return this.y1;
  }, v => {
    this.y1 = v;
  }, true);
...
["b", 1].forEach(prop => _util.default.defineGetterSetter(Line.prototype, prop, function () {
...
Line.transform = (line, matrix) => {
  const a = Point.transform({
    x: line.x1,
    y: line.y1
  }, matrix);
  const b = Point.transform({
    x: line.x2,
    y: line.y2
  }, matrix);
  return {
  ...
  if (args.length >= 3) {
    ...
    if (typeof arg === "string") {
      if (arg.startsWith("#")) {
        ...
        let mul = arg.length >= 7 ? 1 : 17; //console.log('RGBA match:', c, ' mul:', mul);
...
RGBA.fromHex = (hex, alpha = 255) => {
  ...
  const [r, g, b, a] = [...matches].slice(1).map(x => parseInt(x, 16) * mul); //console.log('RGBA.fromHex', { hex, matches, r, g, b, a });
...
RGBA.prototype.toString = function (a) {
  ...
  if (a >= 255) return "rgb(" + this.r + ", " + this.g + ", " + this.b + ")";else return "rgba(" + this.r + ", " + this.g + ", " + this.b + ", " + (a / 255).toFixed(3) + ")";
};
...
RGBA.prototype.toAlpha = function (color) {
  ...
  let dst = RGBA.normalize(src, 1.0, 255); //console.log({ src, dst });
...
let Timer = function Timer(timeout, fn, props = {}, {
  create = setInterval,
...
Timer.once = (timeout, fn, props) => Timer(timeout, fn, props, {
  create: setTimeout,
  destroy: clearTimeout
});

Timer.promise = timeout => new Promise((resolve, reject) => Timer(timeout, resolve, {}, {
  create: setTimeout,
  destroy: id => {
    clearTimeout(id);
    reject();
...
class Element extends Node {
  static create() {
    ...
    let parent = args.shift(); //console.log('Element.create ', { tagName, props, parent });
  ...
  static toObject(elem) {
    ...
    let children = e.children && e.children.length ? {
  ...
  static getRect(elem) {
    ...
    const bbrect = elem.getBoundingClientRect(); //console.log('getRect: ', { bbrect });
  ...
  static rect(elem, options = {}) {
    ...
    r = new Rect(round ? Rect.round(r) : r); //console.log('Element.rect(', element, ') =', r);

    return r;
  }

  static setRect(element, rect, anchor = Anchor.LEFT | Anchor.TOP) {
    const e = typeof element === "string" ? Element.find(element) : element; //console.log('Element.setRect(', element, ',', rect, ', ', anchor, ') ');
  ...
  static move(element, point, pos) {
    ...
    let {
      x = Element.position(element).x,
      y = Element.position(element).y
    } = new Point(rest);
    let to = {
      x,
      y
    ...
    let off; //console.log('Element.move ', { element, to, position });

    const getValue = prop => {
      const property = dom.Element.getCSS(element, prop);
      if (property === undefined) return undefined;
      const matches = /([-0-9.]+)(.*)/.exec(property) || []; //console.log({ match, value, unit });
    ...
    let css = Point.toCSS(current); //console.log("Element.move: ", { position, to, css, off, current });
  ...
  static xpath(elt, relative_to = null) {
    let path = ""; //console.log('relative_to: ', relative_to);
...
Element.cumulativeOffset = (element, relative_to = null) => {
  if (typeof element == "string") element = Element.find(element);
  let p = {
    x: 0,
    y: 0
...
Element.setCSS = (element, prop, value) => {
  if (typeof element == "string") element = Element.find(element);
...
Element.getCSS = (element, property = undefined, receiver = null) => {
  element = typeof element === "string" ? Element.find(element) : element;
  const w = window !== undefined ? window : global.window;
  const d = document !== undefined ? document : global.document; //console.log('Element.getCSS ', { w, d, element });

  let parent = element.parentElement ? element.parentElement : element.parentNode;

  const estyle = _util.default.toHash(w && w.getComputedStyle ? w.getComputedStyle(element) : d.getComputedStyle(element));

  const pstyle = parent && parent.tagName ? _util.default.toHash(w && w.getComputedStyle ? w.getComputedStyle(parent) : d.getComputedStyle(parent)) : {}; //console.log('Element.getCSS ', { estyle, pstyle });

  let style = _util.default.removeEqual(estyle, pstyle);

  let keys = Object.keys(style).filter(k => !/^__/.test(k)); // console.log('style: ', style);
      ...
      if (typeof result == "object") {
        try {
          Object.defineProperty(result, "cssText", {
            get: function () {
              return Object.entries(this).map(([k, v]) => `${_util.default.decamelize(k, "-")}: ${v};\n`).join("");
            },
            enumerable: false
          });
        } catch (err) {}
...
Element.prev_sibling = sib => sib.previousElementSibling;

Element.next_sibling = sib => sib.nextElementSibling;
...
class SVG extends Element {
  static create(name, attr, parent) {
    ...
    _util.default.foreach(attr, (value, name) => svg.setAttribute(_util.default.decamelize(name, "-"), value));
...
ReactComponent.factory = (render_to, root) => {
  ...
  let ret = function render_factory(Tag, {
    parent,
    children,
    ...props
  }, is_root = true) {
...
class Renderer {
  ...
  refresh() {
    ...
    const xpath = Element.xpath(e); //console.log('Renderer.refresh ', { xpath, e });
...
class Select extends React.Component {
  ...
  render() {
    const {
      options,
      ...props
    } = this.props; //console.log('Select.render ', { options, props });
...
ElementRectProxy.prototype = {
  ...
  getRect: function (fn = rect => rect) {
    return fn(Element.rect(this.element, {
      round: false
    }));
  },
  setPos: function (pos) {
...
const ElementPosProps = (element, proxy) => {
  ...
  _util.default.defineGetterSetter(element, "x", () => proxy.getPos().x, x => proxy.setPos({
    x
  }));

  _util.default.defineGetterSetter(element, "x1", () => proxy.getPos().x, value => proxy.setRect(rect => {
    let extend = rect.x - value;
    rect.width += extend;
    return rect;
  }));

  _util.default.defineGetterSetter(element, "x2", () => proxy.getRect(rect => rect.x + rect.width), value => proxy.setRect(rect => {
    let extend = value - (rect.x + rect.w);
    rect.width += extend;
    return rect;
  }));

  _util.default.defineGetterSetter(element, "y", () => proxy.getPos().y, y => proxy.setPos({
    y
  }));

  _util.default.defineGetterSetter(element, "y1", () => proxy.getPos().y, value => proxy.setRect(rect => {
    let y = rect.y - value;
    rect.height += y;
    return rect;
  }));

  _util.default.defineGetterSetter(element, "y2", () => proxy.getRect(rect => rect.y + rect.height), value => proxy.setRect(rect => {
...
const ElementSizeProps = (element, proxy) => {
  ...
  _util.default.defineGetterSetter(element, "w", () => proxy.getRect().width, width => proxy.setSize({
    width
  }));

  _util.default.defineGetterSetter(element, "width", () => proxy.getRect().width, width => proxy.setSize({
    width
  }));

  _util.default.defineGetterSetter(element, "h", () => proxy.getRect().height, width => proxy.setSize({
    height
  }));

  _util.default.defineGetterSetter(element, "height", () => proxy.getRect().height, width => proxy.setSize({
    height
  }));
};
...
const isPointList = inst => {};

exports.isPointList = isPointList;
...
const isReactComponent = inst => {};

exports.isReactComponent = isReactComponent;

const isRenderer = inst => {};
