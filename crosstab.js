...
(function (window, define) {
  define('crosstab', (require, exports, module) => {
    ...
    //--- Setup Events ---
    let eventHandler = util.createEventHandler();
    ...
    function setLocalStorageItem(key, data) {
      let storageItem = { id: crosstab.id, data, timestamp: util.now() };

      localStorage.setItem(key, JSON.toString(storageItem));
    }

    function getLocalStorageItem(key) {
      let item = getLocalStorageRaw(key);
      return item.data;
    }

    function getLocalStorageRaw(key) {
      let json = localStorage ? localStorage.getItem(key) : null;
    ...
    util.events.once('setupComplete', () => {
      setupComplete = true;
    });
    ...
    //--- Tab Setup ---
    //3 second keepalive
    let TAB_KEEPALIVE = 3 * 1000;
    //5 second timeout
    let TAB_TIMEOUT = 5 * 1000;
    //500 ms ping timeout
    let PING_TIMEOUT = 500;

    function getStoredTabs() {
      let storedTabs = getLocalStorageItem(util.keys.TABS_KEY);
    ...
    function keepalive() {
      ...
      let myTab = { id: crosstab.id, lastUpdated: now };
