...
export class EagleNodeList {
  constructor(
    ...
    ref,
    pred,
    getOrCreate = EagleElement.get
  ) {
    //console.log('EagleNodeList.constructor', { owner: owner.raw, ref, pred });
    if(Util.isObject(owner) && !('raw' in owner))
      throw new Error('raw owner');
    ...
    this.pred =
      typeof pred == 'function' ? pred : () => true;
  }
  ...
  item(pos) {
    ...
    if(
      raw &&
      Util.isObject(raw[pos]) &&
      'tagName' in raw[pos]
    ) {
      owner.document.raw2element.map.delete(raw[pos]);

      let element = this.getOrCreate(
        owner.document,
        ref.down(pos)
      );
      // if(pred(element))
  ...
  *[Symbol.iterator]() {
  ...
  remove(cond) {
    ...
    for(let i = raw.length - 1; i >= 0; i--)
      if(pred(raw[i], i, this) && cond(raw[i], i, this))
        raw.splice(i, 1);
  ...
  static create(owner, ref, pred, getOrCreate) {
    let instance = new EagleNodeList(
      owner,
      ref,
      pred,
      getOrCreate
    ...
    return new Proxy(instance, {
      ...
      get(target, prop, receiver) {
        ...
        if(
          typeof EagleNodeList.prototype[prop] == 'function'
        )
...
EagleNodeList.prototype[Symbol.toStringTag] =
  'EagleNodeList';
