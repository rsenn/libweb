...
Util.extend(EagleNodeList.prototype, {
  *[Symbol.iterator]() {
    ...
    for(let i = 0; i < list.length; i++)
      yield makeEagleElement(instance, this.ref.down(i));
  },
  ...
  iterator() {
    ...
    return function*() {
      ...
      for(let i = 0; i < list.length; i++)
        yield makeEagleElement(instance, instance.ref.down(i));
    };
  },
  *entries() {
    const instance = this;
...
EagleNodeList.make = function() {
  ...
  let down = [];
  if('ref' in owner && 'length' in args[0])
    ref = EagleRef(owner.ref.root, args.shift());
  ...
  return new Proxy(instance, {
    ...
    get(target, prop, receiver) {
      ...
      if(typeof Ctor.prototype[prop] == 'function')
        return Ctor.prototype[prop].bind(instance);
      let list = instance.ref.dereference();
      if(prop == 'find')
        return name => {
          ...
          return idx == -1
            ? null
            : makeEagleElement(instance, instance.ref.down(idx));
        };
      if(prop == 'entries')
        return () => list.map((item, i) => [item.attributes.name, item]);
      if(/^[0-9]+$/.test(prop + '')) {
