...
export class EagleNode {
  ...
  constructor(owner, ref, raw) {
    ...
    if(!(ref instanceof EagleReference))
      ref = new EagleRef(
        owner && 'ref' in owner ? owner.ref.root : owner,
        [...ref]
      );
  ...
  get project() {
    if(Util.className(this.owner) == 'EagleProject')
      return this.owner;
  ...
  get childConstructor() {
    ...
    if(
      Util.fnName(protos[0].constructor) == 'EagleDocument'
    )
      protos.shift();
      ...
      for(let xpath of fields) {
        try {
          ...
          let path = new Pointer(
            [
              ...new ImmutableXPath(xpath).toPointer(raw)
            ].concat(['children'])
          );
          ...
          maps[key] =
            ['sheets', 'connects', 'plain'].indexOf(key) !=
            -1
              ? lists[key]
              : () =>
                  EagleNodeMap.create(
  ...
  initRelation(key, handler, fn) {
    ...
    trkl.bind(this, key, v => {
      if(v !== undefined)
        return handler(typeof v == 'string' ? v : v.name);
      let value =
        handler() ||
        elem.attrMap[key] ||
        elem.attributes[key];
  ...
  static makePredicate(predicate) {
    ...
    if(pred instanceof RegExp) {
      ...
      let keys = Util.isArray(pred)
        ? pred
        : Object.keys(pred);
      let values = keys.reduce(
        (acc, key) => [...acc, pred[key]],
        []
      );
  ...
  *getAll(predicate, transform) {
    ...
    let cond = pred;
    for(let [v, p] of deep.iterate(
      this.raw,
      cond /*, deep.RETURN_VALUE_PATH*/
    )) {
      yield transform(
        v,
        [...this.path, ...p],
  ...
  getMap(entity) {
    ...
    if(a && a.children)
      return new Map([...a.children].map(e => [e.name, e]));
    return null;
  }
  ...
  get names() {
    ...
    do {
      ...
      if(node.attributes.name !== undefined)
        names.push([node.tagName, node.attributes.name]);
      ref = ref.up(2);
  ...
  get nextSibling() {
    ...
    return ref
      ? new this.constructor[Symbol.species](this, ref)
      : null;
  }
  ...
  get parentNode() {
    ...
    return this.constructor[Symbol.species].get(
      doc,
      ref.up(2)
    );
  }
  ...
  inspect(depth, options) {
    ...
    const attributeList =
      attributeLists[tagName] || Object.keys(attributes);
    ...
    if(this.filename)
      ret = concat(ret, ` filename="${this.filename}"`);
    if(numChildren > 0) {
      if(depth < 1) {
        ret += (
          '\n' + xml.write(children, options.depth - depth)
        ).replace(/\n/g, '\n  ');
        ret += '\n';
      } else {
        ret += concat(
  ...
  lookup(xpath, t = (o, p, v) => [o, p]) {
    /* if(!(xpath instanceof ImmutableXPath)) */ xpath =
      new ImmutableXPath(xpath);
  ...
  getBounds(pred = e => true) {
    ...
    if(this.children && this.children.length) {
      for(let element of this.getAll(
        e => e.tagName !== undefined && pred(e)
      )) {
        let g = element?.geometry;
        if(g) {
          let bound =
            typeof g.bbox == 'function' ? g.bbox() : g;
  ...
  *iterator(...args) {
    let predicate =
      typeof args[0] == 'function'
        ...
        : arg => true;
    let path =
      (Util.isArray(args[0]) && args.shift()) || [];
    let t =
      typeof args[0] == 'function'
        ? args.shift()
        : ([v, l, d]) => [
    ...
    let owner =
      Util.isObject(this) && 'owner' in this
        ? this.owner
        : this;
    ...
    for(let [v, l] of deep.iterate(node, (v, p) =>
      ...
      if(!(l instanceof ImmutablePath))
        l = new ImmutablePath(l);
      if(
        typeof v == 'object' &&
        v !== null &&
        'tagName' in v
  ...
  static inspect = (
    ...
    x = Object.entries((r && r.attributes) || {}).map(
      ([key, value]) =>
        text(key, 33) + text(s, 0, 37) + text(value, 1, 36)
    );
    x.unshift(r.tagName);
    let [p, ...arr] = x;
