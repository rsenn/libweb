...
export class LibraryRenderer extends EagleSVGRenderer {
  ...
  constructor(doc, factory) {
    ...
    //this.setPalette(LibraryRenderer.palette);
    this.palette = LibraryRenderer.palette;
    //Util.log('found:', new ImmutablePath([...doc.path, 'children', { tagName: 'eagle' }, 'children', { tagName: 'drawing' }, 'children', { tagName: 'Library' }]));

    //Util.log('LibraryRenderer.constructor(', doc, factory, ')');
  }
  ...
  renderCollection(collection, parent, opts) {
    ...
    this.debug(`LibraryRenderer.renderCollection`, arr, opts);
  ...
  renderItem(item, parent, opts = {}) {
    ...
    switch (item.tagName) {
      ...
      case 'pin': {
        ...
        let veclen = LibraryRenderer.pinSizes[length] * 2.54;
  ...
  renderNet(net, parent) {
    this.debug(`LibraryRenderer.renderNet`, { net, parent });
  ...
  renderSheet(sheet, parent) {
    ...
    this.debug(`LibraryRenderer.renderSheet`, { sheet, parent, transform });

    let instances = sheet.instances; //find('instances');

    this.debug(`LibraryRenderer.renderSheet`, sheet);
  ...
  renderInstance(instance, parent, opts = {}) {
    this.debug(`LibraryRenderer.renderInstance`, instance);
    ...
    this.debug(`LibraryRenderer.renderInstance`, { x, y, transform });
