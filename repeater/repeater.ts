 ...
 */
class RepeaterController<T, TReturn = any, TNext = unknown> implements AsyncGenerator<T, TReturn, TNext> {
  ...
  constructor(private executor: RepeaterExecutor<T, TReturn, TNext>, private buffer: RepeaterBuffer) {}
   ...
   */
  private unwrap(value: Promise<T | TReturn | undefined> | T | TReturn | undefined): Promise<IteratorResult<T, TReturn>> {
  ...
  private push(value: PromiseLike<T> | T): Promise<TNext | undefined> {
    ...
    let valueP: Promise<T | undefined> = this.pending === undefined ? Promise.resolve(value) : this.pending.then(() => value);
  ...
  private stop(err?: any): void {
    ...
    if(this.pullQueue.length) {
      ...
      for(const pull of this.pullQueue) {
        const execution: Promise<TReturn | undefined> = this.pending === undefined ? this.consume() : this.pending.then(() => this.consume());
  ...
  next(value?: PromiseLike<TNext> | TNext): Promise<IteratorResult<T, TReturn>> {
    ...
    if(!this.buffer.empty) {
      const result = this.unwrap(this.buffer.remove() as Promise<T | undefined>);
  ...
  throw(err: any): Promise<IteratorResult<T, TReturn>> {
    if(this.state <= RepeaterState.Initial || this.state >= RepeaterState.Stopped || !this.buffer.empty) {
      this.finish();
      if(this.err == null) {
        this.err = err;
      }
...
const controllers = new WeakMap<Repeater<any, any, any>, RepeaterController<any, any, any>>();
...
export class Repeater<T, TReturn = any, TNext = unknown> {
  constructor(executor: RepeaterExecutor<T, TReturn, TNext>, buffer: RepeaterBuffer = new FixedBuffer(0)) {
...
function asyncIterators(contenders: Iterable<any>, options: { yieldValues?: boolean; returnValues?: boolean }): AsyncIterator<any>[] {
...
function race<T>(contenders: Iterable<T>): Repeater<T extends AsyncIterable<infer U> | Iterable<infer U> ? (U extends PromiseLike<infer V> ? V : U) : never> {
...
function merge<T>(contenders: Iterable<T>): Repeater<T extends AsyncIterable<infer U> | Iterable<infer U> ? (U extends PromiseLike<infer V> ? V : U) : T extends PromiseLike<infer U> ? U : T> {
