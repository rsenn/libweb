...
function useRepeater(buffer) {
    var _a = __read(useState(function () { return createPrimedRepeater(buffer); }), 1), tuple = _a[0];
    return tuple;
}
function useAsyncIter(callback, deps) {
    if (deps === void 0) { deps = []; }
    var _a = __read(useRepeater(), 2), repeater = _a[0], push = _a[1];
    var _b = __read(useState(function () { return callback(repeater); }), 1), iter = _b[0];
    useEffect(function () {
        push(deps);
...
function useResult(callback, deps) {
    var _this = this;
    var iter = useAsyncIter(callback, deps);
...
function useValue(callback, deps) {
    var result = useResult(callback, deps);
