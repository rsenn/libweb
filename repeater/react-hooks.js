...
export function useRepeater(buffer) {
    let _a = __read(useState(()=>createPrimedRepeater(buffer)), 1), tuple = _a[0];
    return tuple;
}
export function useAsyncIter(callback, deps) {
    ...
    let _a = __read(useRepeater(), 2), repeater = _a[0], push = _a[1];
    let _b = __read(useState(()=>callback(repeater)), 1), iter = _b[0];
    useEffect(()=>{
        push(deps);
    }, __spread([
        push
...
export function useResult(callback, deps) {
    ...
    let _a = __read(useState(), 2), result = _a[0], setResult = _a[1];
    useEffect(()=>{
        let mounted = true;
        (function() {
