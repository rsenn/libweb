...
Shash.prototype.check = function () {
  ...
  //Clear grids
  let i = Math.ceil(this.size.x / this.gridSize.x),
  ...
  i = this.grid.length;
  while(i--) {
    let gridx = this.grid[i],
      iLUB = i !== this.grid.length - 1; //I: not last element
    j = gridx.length;
    while(j--) {
      let grid = gridx[j], //Focused grid cell
        k = grid.length,
      ...
      while(k--) {
        ...
        if(iLUB) {
          ...
          let ogridx = this.grid[i + 1]; //Row of grids to the right of focused grid
