...
const pushValues = (node, values) => {
  ...
  for(let k = keys(node), { length } = k, i = 0; i < length; i++) pushValues(node[k[i]], values);
};
...
export default class TrieMap {
  ...
  clear() {
    for(const key in this.root) delete this.root[key];
  }
  ...
  *entries() {
    ...
    let i = 0;
    while(i < all.length) yield all[i++];
  }

  forEach(callback, context) {
    const all = [];
    pushKeyValues(this.root, all);
    for(let { length } = all, i = 0; i < length; i++) callback.call(context, all[i][1], all[i][0], this);
  }
  ...
  set(key, value) {
    if(this.ignoreCase) key = key.toLowerCase();
    let { root } = this;
    for(let { length } = key, i = 0; i < length; i++) root = root[key[i]] || (root[key[i]] = {});
    root[VALUE] = value;
    return this;
  }
