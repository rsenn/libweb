export default class DenseSpatialHash2D {
	...
	add( x, y, w, h ) {
		...
		this._addToBuckets( id, minX, minY, maxX, maxY )
		return id
	}

	_addToBuckets( id, minX, minY, maxX, maxY ) {
	...
	addRect( id, dx, dy, dw, dh ) {
		if ( id >= 0 && id < this.rects.length ) {
			...
			rect[7] = maxY
			if ( changed ) {
				this._addToBuckets( id, minX, maxX, maxY )
			}
			return true
		} else {
			return false
