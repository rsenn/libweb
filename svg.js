 ...
 * @license MIT
 *
 * BUILT: Tue May 14 2024 11:37:35 GMT+0200 (Mitteleuropäische Sommerzeit)
 */ const methods$1 = {};
const names = [];
...
class Runner extends EventTarget {
  ...
  static sanitise(duration, delay, when) {
    var _duration, _delay;
    // Initialise the default parameters
    let times = 1;
    let swing = false;
    let wait = 0;
    duration = (_duration = duration) != null ? _duration : timeline.duration;
    delay = (_delay = delay) != null ? _delay : timeline.delay;
    when = when || 'last';

    // If we have an object, unpack the values
    if(typeof duration === 'object' && !(duration instanceof Stepper)) {
      var _duration$delay, _duration$when, _duration$times, _duration$wait, _duration$duration;
      delay = (_duration$delay = duration.delay) != null ? _duration$delay : delay;
      when = (_duration$when = duration.when) != null ? _duration$when : when;
      swing = duration.swing || swing;
      times = (_duration$times = duration.times) != null ? _duration$times : times;
      wait = (_duration$wait = duration.wait) != null ? _duration$wait : wait;
      duration = (_duration$duration = duration.duration) != null ? _duration$duration : timeline.duration;
    }
    return {
      duration: duration,
      delay: delay,
...
class Text extends Shape {
  ...
  constructor(node, attrs = node) {
    ...
    this.dom.leading = (_this$dom$leading = this.dom.leading) != null ? _this$dom$leading : new SVGNumber(1.3); // store leading value for rebuilding
