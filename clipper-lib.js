...
let Inherit = function(ce, ce2) {
  ...
  if(typeof Object.getOwnPropertyNames === 'undefined') {
    ...
    for(let i = 0; i < props.length; i++)
      if(typeof Object.getOwnPropertyDescriptor(ce.prototype, props[i]) ===
        'undefined'
      )
...
ClipperLib.ClipperBase.prototype.PointOnLineSegment = function(pt,
  linePt1,
  linePt2
) {
...
ClipperLib.ClipperBase.prototype.FindNextLocMin = function(E) {
  let E2;
  for(;;) {
    while(ClipperLib.FPoint.op_Inequality(E.Bot, E.Prev.Bot) ||
...
ClipperLib.ClipperBase.prototype.ProcessBound = function(E,
  LeftBoundIsForward
) {
  let EStart;
  let Result = E;
  ...
  if(LeftBoundIsForward) {
    while(Result.Top.Y === Result.Next.Bot.Y &&
      ...
      Result = Result.Next;
    if(Result.Dx === ClipperLib.ClipperBase.horizontal &&
      Result.Next.OutIdx !== ClipperLib.ClipperBase.Skip
    ) {
...
ClipperLib.ClipperBase.prototype.Pt2IsBetweenPt1AndPt3 = function(pt1,
  pt2,
  pt3
) {
  if(ClipperLib.FPoint.op_Equality(pt1, pt3) ||
...
ClipperLib.Clipper.prototype.InsertLocalMinimaIntoAEL = function(botY) {
  ...
  while(this.PopLocalMinima(botY, lm)) {
    ...
    if(lb.OutIdx >= 0 &&
      ...
      ClipperLib.ClipperBase.SlopesEqual5(lb.PrevInAEL.Curr,
        lb.PrevInAEL.Top,
        lb.Curr,
        lb.Top
      ) &&
    ...
    if(lb.NextInAEL !== rb) {
      if(rb.OutIdx >= 0 &&
        ...
        ClipperLib.ClipperBase.SlopesEqual5(rb.PrevInAEL.Curr,
          rb.PrevInAEL.Top,
          rb.Curr,
          rb.Top
        ) &&
...
ClipperLib.Clipper.prototype.AddLocalMinPoly = function(e1, e2, pt) {
  ...
  if(prevE !== null &&
    ...
    if(xPrev === xE &&
      e.WindDelta !== 0 &&
      prevE.WindDelta !== 0 &&
      ClipperLib.ClipperBase.SlopesEqual5(new ClipperLib.FPoint2(xPrev, pt.Y),
        prevE.Top,
...
ClipperLib.Clipper.prototype.HorzSegmentsOverlap = function(seg1a,
  seg1b,
  seg2a,
  seg2b
) {
...
ClipperLib.Clipper.prototype.OutRec1RightOfOutRec2 = function(outRec1,
  outRec2
) {
  do {
    ...
    if(e1.PolyTyp !== e2.PolyTyp) {
      ...
      switch (this.m_ClipType) {
        ...
        case ClipperLib.ClipType.ctUnion:
          ...
          break;
        case ClipperLib.ClipType.ctDifference:
          if((e1.PolyTyp === ClipperLib.PolyType.ptClip &&
              e1Wc2 > 0 &&
              e2Wc2 > 0) ||
...
ClipperLib.Clipper.prototype.GetMaximaPair = function(e) {
  if(ClipperLib.FPoint.op_Equality(e.Next.Top, e.Top) &&
    e.Next.NextInLML === null
  ) {
    return e.Next;
  }
  if(ClipperLib.FPoint.op_Equality(e.Prev.Top, e.Top) &&
...
ClipperLib.Clipper.prototype.FixupIntersectionOrder = function() {
  ...
  for(let i = 0; i < cnt; i++) {
    ...
    this.SwapPositionsInSEL(this.m_IntersectList[i].Edge1,
  ...
  while(e !== null) {
    if(this.IsIntermediate(e, topY)) {
      ...
      if(ePrev !== null &&
        ...
        ePrev.OutIdx >= 0 &&
        ePrev.Curr.Y === ePrev.Top.Y &&
        ClipperLib.ClipperBase.SlopesEqual5(e.Curr,
          e.Top,
          ePrev.Curr,
          ePrev.Top
        ) &&
...
ClipperLib.Clipper.prototype.FixupOutPolygon = function(outRec) {
  ...
  for(;;) {
    ...
    if(ClipperLib.FPoint.op_Equality(pp.Pt, pp.Next.Pt) ||
...
ClipperLib.Clipper.prototype.JoinHorz = function(op1,
  op1b,
  op2,
  op2b,
  Pt,
  ...
  for(let i = 1; i <= cnt; ++i) {
    ...
    if(ip.Y < pt.Y !== ipNext.Y < pt.Y) {
      if(ip.X >= pt.X) {
        ...
        var d =
          (ip.X - pt.X) * (ipNext.Y - pt.Y) - (ipNext.X - pt.X) * (ip.Y - pt.Y);
        if(d === 0) return -1;
        else if(d > 0 === ipNext.Y > ip.Y) result = 1 - result;
      }
    }
...
ClipperLib.Clipper.prototype.FixupFirstLefts1 = function(OldOutRec,
  NewOutRec
) {
...
ClipperLib.Clipper.prototype.FixupFirstLefts2 = function(innerOutRec,
  outerOutRec
) {
...
ClipperLib.Clipper.prototype.FixupFirstLefts3 = function(OldOutRec,
  NewOutRec
) {
  ...
  while(i < this.m_PolyOuts.length) {
    ...
    do //for each Pt in Polygon until duplicate found do ...
    {
      ...
      while(op2 !== outrec.Pts) {
        if(ClipperLib.FPoint.op_Equality(op.Pt, op2.Pt) &&
          op2.Next !== op &&
          op2.Prev !== op
        ) {
...
ClipperLib.Clipper.CleanPolygon = function(path, distance) {
  ...
  while(op.Idx === 0 && op.Next !== op.Prev) {
    if(ClipperLib.Clipper.PointsAreClose(op.Pt, op.Prev.Pt, distSqrd)) {
      ...
      op = ClipperLib.Clipper.ExcludeOp(op);
      cnt -= 2;
    } else if(ClipperLib.Clipper.SlopesNearCollinear(
        op.Prev.Pt,
        op.Pt,
        op.Next.Pt,
        distSqrd
...
ClipperLib.Clipper.Minkowski = function(pattern, path, IsSum, IsClosed) {
  ...
  if(IsSum)
    for(var i = 0; i < pathCnt; i++) {
      ...
      for(var j = 0, jlen = pattern.length, ip = pattern[j];
        j < jlen;
        j++, ip = pattern[j]
      )
...
ClipperLib.Clipper.MinkowskiSum = function(pattern,
  path_or_paths,
  pathIsClosed
) {
  ...
  for(let i = 0; i < paths.length; ++i) {
    let tmp = ClipperLib.Clipper.Minkowski(pattern,
      paths[i],
      true,
      pathIsClosed
    );
...
ClipperLib.Clipper.PolyTreeToPaths = function(polytree) {
  ...
  ClipperLib.Clipper.AddPolyNodeToPaths(polytree,
...
ClipperLib.Clipper.AddPolyNodeToPaths = function(polynode, nt, paths) {
  ...
  for(let $i3 = 0, $t3 = polynode.Childs(), $l3 = $t3.length, pn = $t3[$i3];
    $i3 < $l3;
    $i3++, pn = $t3[$i3]
  )
...
ClipperLib.ClipperOffset.prototype.AddPath = function(path,
  joinType,
  endType
) {
  ...
  if(this.m_lowest.X < 0)
    this.m_lowest = new ClipperLib.FPoint2(this.m_polyNodes.ChildCount() - 1,
      k
    );
  else {
    let ip = this.m_polyNodes.Childs()[this.m_lowest.X].m_polygon[
      this.m_lowest.Y
    ];
...
ClipperLib.ClipperOffset.prototype.AddPaths = function(paths,
  joinType,
  endType
) {
...
ClipperLib.ClipperOffset.prototype.DoOffset = function(delta) {
  ...
  for(var i = 0; i < this.m_polyNodes.ChildCount(); i++) {
    ...
    for(var j = 0; j < len - 1; j++)
      this.m_normals.push(ClipperLib.ClipperOffset.GetUnitNormal(
          this.m_srcPoly[j],
          this.m_srcPoly[j + 1]
        )
      );
    ...
    if(node.m_endtype === ClipperLib.EndType.etClosedPolygon) {
      ...
      for(var j = len - 1; j > 0; j--)
        this.m_normals[j] = new ClipperLib.FPoint2(-this.m_normals[j - 1].X,
      ...
      if(node.m_endtype === ClipperLib.EndType.etOpenButt) {
        var j = len - 1;
        pt1 = new ClipperLib.FPoint2(this.m_srcPoly[j].X + this.m_normals[j].X * delta,
        ...
        k = len - 2;
        this.m_sinA = 0;
        this.m_normals[j] = new ClipperLib.FPoint2(-this.m_normals[j].X,
          -this.m_normals[j].Y
        );
      ...
      this.m_normals[0] = new ClipperLib.FPoint2(-this.m_normals[1].X,
        -this.m_normals[1].Y
      );
      k = len - 1;
      for(var j = k - 1; j > 0; --j)
        k = this.OffsetPoint(j, k, node.m_jointype);
      if(node.m_endtype === ClipperLib.EndType.etOpenButt) {
        pt1 = new ClipperLib.FPoint2(this.m_srcPoly[0].X - this.m_normals[0].X * delta,
...
ClipperLib.ClipperOffset.prototype.Execute = function() {
  ...
  if(!ispolytree) {
    ...
    if(delta > 0) {
      clpr.Execute(ClipperLib.ClipType.ctUnion,
        solution,
