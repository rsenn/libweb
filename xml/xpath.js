...
export class MutableXPath extends MutablePath {
  ...
  static [Symbol.hasInstance](instance) {
    return (
      ['MutableXPath', 'ImmutableXPath', 'XPath'].indexOf(
        className(instance),
      ) != -1
    );
  }
  ...
  static partMatcher(part) {
    ...
    } else if(isObject(part) && typeof part.tagName == 'string') {
  ...
  static from(path, obj) {
    ...
    if(absolute)
      if(path.length == 0 || path[0] !== '') path = [''].concat(path);
    let o = obj;
    let n;
    let a =
      isObject(path) && typeof path.toArray == 'function'
        ? path.toArray()
        : [...path];
    let e;
    let s = [];
    ...
    for(; i < a.length; i++) {
      ...
      else if(isObject(p) && isArray(o))
        p = o.findIndex(item => item.tagName === p.tagName);
      ...
      if(p == 'children' || isArray(e)) {
        ...
        if(isObject(e.attributes) && e.attributes.name) {
          ...
          if(siblings[p])
            p = siblings
              .slice(0, p)
              .filter(([idx, tagName]) => tagName == e.tagName).length;
          else x = '';
          //console.log('', { p, x });
          x += '[' + (p + 1).toString(10) + `]`;
        }
    ...
    let r = new ImmutableXPath(
      Object.setPrototypeOf(s, ImmutableXPath.prototype),
      absolute,
      obj,
    );
...
*/
  ...
  static parse(l) {
    ...
    if(l.indexOf('children') != -1) {
      l = l.filter(
        p => !(ImmutablePath.isChildren ?? MutableXPath.isChildren)(p),
      );
    }
    //console.log('MutableXPath.parse', { l });
  ...
  toCode(name = '', opts = {}) {
    ...
    let r = this.toArray();
    r = r.reduce(
      (acc, part, i) => acc + (y(i) ? spacing : '') + partToStr(part),
      name,
    );
    if(fn) {
      const code = `${name} => ${r}`;
    ...
    function partToStr(part) {
      ...
      if(typeof part == 'number' || isNumeric(part)) return `[${part}]`;
  ...
  /*  [Symbol.inspect ?? Symbol.for('nodejs.util.inspect.custom')](c) {
    ...
    let n = className(this).replace(/Immutable/, '');
...
export const findXPath = (
  xpath,
  flat,
  { root, recursive = true, entries = false },
) => {
  let r = [];
...
export const XPath = immutableClass(MutableXPath);
