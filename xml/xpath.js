...
export class MutableXPath extends MutablePath {
  ...
  static [Symbol.hasInstance](instance) {
    return (['MutableXPath', 'ImmutableXPath', 'XPath'].indexOf(
        Util.className(instance)
      ) != -1
    );
  }
  ...
  static from(path, obj) {
    ...
    let n;
    let a =
      Util.isObject(path) && typeof path.toArray == 'function'
        ? path.toArray()
        : [...path];
    let e;
    let s = [];
    ...
    for(; i < a.length; i++) {
      ...
      else if(Util.isObject(p) && Util.isArray(o))
    ...
    let r = new ImmutableXPath(Object.setPrototypeOf(s, ImmutableXPath.prototype),
      absolute,
      obj
    );
...
*/
  ...
  static parse(l) {
    l = Util.isArray(l)
      ? l
      : l
          .split(new RegExp(`\\s?[.\\/${this.CHILDREN_GLYPH}]\\s?`, 'g'))
          .map(p => (isNaN(+p) ? p : +p));
  ...
  toString(sep = '/',
    childrenSym = this.constructor.CHILDREN_GLYPH,
    tfn = text => text
  ) {
    ...
    let s = r.join('/');
    return (sep + s)
      .replace(new RegExp(ctor.CHILDREN_GLYPH + '(//*)', 'g'), '$1')
      .replace(/(\/+)/g, '/');
  }
  ...
  toCode(name = '', opts = {}) {
    ...
    if(fn && name == '') name = 'arg';
    let r = this.toArray();
    r = r.reduce((acc, part, i) => acc + (y(i) ? spacing : '') + partToStr(part),
      name
    );
    if(fn) {
      const code = `${name} => ${r}`;
    ...
    function partToStr(part) {
      ...
      if(Util.isArray(part)) {
        ...
        const attrs = part.attributes
          ...
          : [];
        const pred = `({tagName${attrs.length ? ',attributes' : ''}}) => ${[
          cond,
          ...attrs
        ].join(' & ')}`;
        part = `find(${pred})`;
  ...
  [Symbol.for('nodejs.util.inspect.custom')](c) {
    ...
    c =
      typeof c == 'function'
        ? c
        : (text, ...colors) => `\x1b[${colors.join(';')}m${text}\x1b[0m`;
