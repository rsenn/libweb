...
export function Matrix(arg) {
  ...
  if(arg && typeof arg == "object") {
    if(arg.xx !== undefined && arg.yx !== undefined && arg.xy !== undefined && arg.yy !== undefined && arg.x0 !== undefined && arg.y0 !== undefined) {
      ...
      ret.x0 = arg.x0;
      ret.y0 = arg.y0;
    } else if(Util.isBrowser() && arg instanceof DOMMatrix) {
      ret.xx = arg.a;
      ret.yx = arg.b;
      ret.xy = arg.c;
      ret.yy = arg.d;
...
Matrix.multiply = function(a, b, fn = m => m) {
  a = fn(a);
  b = fn(b);
  return fn({
...
Matrix.identity = () => ({ xx: 1, xy: 0, yx: 0, yy: 1, x0: 0, y0: 0 });

Matrix.translate = (m, tx, ty) => {
  let matrix = m instanceof Matrix ? m : new Matrix(m);
  return {
    xx: matrix.xx,
    xy: matrix.xy,
    yx: matrix.yx,
...
Matrix.prototype.translate = function(tx, ty) {
  const m = Matrix.init_translate(tx, ty);
...
Matrix.scale = (m, sx, sy) => Matrix.multiply(m, Matrix.init_scale(sx, sy));
...
Matrix.rotate = (m, rad) => Matrix.multiply(m, Matrix.init_rotate(rad));
...
export class Node {
  ...
  static attrs(node) {
    return node.attributes && node.attributes.length > 0
      ? Array.from(node.attributes).reduce((acc, attr) => ({ ...acc, [attr.name]: isNaN(parseFloat(attr.value)) ? attr.value : parseFloat(attr.value) }), {})
      : {};
  }
}
