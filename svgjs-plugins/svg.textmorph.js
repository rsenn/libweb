...
SVG.SVGFont = SVG.invent({
  create(source, cb, lazy, undefined) {
  ...
  extend: {
    ...
    getPathArray(glyphs, cb) {
      ...
      for(var i = 0, len = glyphs.length; i < len; ++i) {
        ...
        if(box.height && box.width) p.size(box.width * scale, -box.height * scale);
...
SVG.MorphText = SVG.invent({
  ...
  extend: {
    ...
    text(glyphs) {
      ...
      var font = svgFont.fontSize(this.size).getPathArray(
        glyphs,
        function(pathArray, index) {
...
SVG.extend(SVG.FX, {
  text(glyphs) {
    ...
    var font = svgFont.fontSize(this.target.size).getPathArray(
      glyphs,
      function(pathArray, index) {
