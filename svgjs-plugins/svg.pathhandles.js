...
(function (SVG, undefined) {
  function PathSegmentHandle(pos, seg, handler, index) {
    ...
    new SVG.Set([this.p1, this.cp1, this.cp2, this.p2]).on(
      'dragmove._handles',
      function() {
  ...
  HandlesHandler.prototype.init = function(value, options) {
    ...
    for(i = 0, len = arr.length; i < len; ++i) {
      if(i) {
        pos = [arr[i - 1][arr[i - 1].length - 2], arr[i - 1][arr[i - 1].length - 1]];
      }
  ...
  HandlesHandler.prototype.handler = function() {
    ...
    this.handles.forEach(function (el, index) {
      ...
      if(index) {
        pos = [arr[index - 1][arr[index - 1].length - 2], arr[index - 1][arr[index - 1].length - 1]];
      }
