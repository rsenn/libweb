...
export function DereferenceError(object, member, pos, prev, locator) {
  ...
  let stack = Util.getCallerStack()
    ...
    .map(frame => {
      ...
      if(method)
        method =
          (frame.getTypeName() || Util.className(frame.getThis())) +
          '.' +
          method;
  ...
  return Object.assign(error,
    { object, member, pos, locator },
    {
      message: `Error dereferencing ${Util.className(
          object
        )} @ ${MutablePath.prototype.toString.call(locator,
          '/',
...
DereferenceError.prototype.toString = function() {
  ...
  return `${message}\n${Util.inspect({ object, member, pos, locator, stack },
...
export class MutablePath extends Array {
  ...
  constructor(p = [], opts = {}) {
    ...
    if(specialFields.length > 0)
      Util.define(this, 'specialFields', specialFields);
    ...
    if(typeof path != 'number') {
      ...
      for(let i = 0; i < len; i++) {
        ...
        if(typeof part == 'string' &&
          ((part && part.codePointAt && part.codePointAt(0) >= 256) ||
            part == 'children')
        ) {
          part = 'children';
        } else if(typeof part == 'number' ||
          (typeof part == 'string' && !isNaN(part))
        ) {
          part = +part;
  ...
  static partToString(a,
    ...
    let s = '';
    let part = a.shift();
    if(typeof part == 'function' && part.object !== undefined)
      part = part.object;
  ...
  get lastId() {return this.length - 1; }
  get last() {return this[this.length - 1]; }
  get first() {return this[0]; }

  get nextSibling() { return this.right(); }
  get prevSibling() { return this.left(); }
  get parent() { return this.up(1); }
  get parentNode() { return this.up(2); }
  get firstChild() { return this.nthChild(0); }
  get lastChild() { return this.nthChild(-1); }
  get depth() { return this.length; }
  ...
  apply(obj, noThrow) {
    ...
    if(a.o == null && !noThrow)
      throw new DereferenceError(obj, a.i, a.n, a.p, this);
    return a.o;
  }
  ...
  toSource(opts = {}) {
    ...
    return `[${r
      .map(p =>
        typeof p == 'number' ? p : typeof p == 'string' ? `'${p}'` : p
      )
      .join(sep)}]`;
  }
  toCode(name) {
    return this.reduce((acc, part) => acc + (Util.isNumeric(part) ? `[${part}]` : `.${part}`),
      name || ''
    );
  }
  toReduce(name = '') {
  ...
  [Symbol.for('nodejs.util.inspect.custom')]() {
    ...
    if(Util.stripAnsi(p).length > max) {
      ...
      p =
        t(`... ${num} components ...`, 1, 36) +
        ' | ' +
        p.substring(pos, p.length);
    }

    return color
      ? `\x1b[1;${c}m${n.replace(/^Immutable/, '')}\x1b[1;30m ${p}\x1b[0m`
      : p;
  }
  ...
  slice(start = 0, end = this.length) {
    ...
    let r = Array.prototype.slice.call(MutablePath.prototype.toArray.call(this),
      start,
      end
    );
  ...
  filter(fn) {
    ...
    let ret = [];
    for(let i = 0; i < this.length; i++)
      if(fn(this.at(i), i, this)) ret.push(this[i]);
    return Object.setPrototypeOf(ret, ctor.prototype);
  }
  ...
  toArray(skip = true, n = 1) {
    ...
    for(; i < this.length; i += n) {
      if(n > 1)
        ret = ret.concat(Array.prototype.slice.call([...this], i, i + 2));
...
Util.defineGetter(Path, Symbol.species, () => ImmutablePath);
