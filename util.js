...
Util.toSource = function(arg, opts = {}) {
  ...
  if(typeof arg == 'string') return Util.colorText(`'${arg}'`, 1, 36);
  if(arg && arg.x !== undefined && arg.y !== undefined)
    return `[${Util.colorText(arg.x,1,32)},${Util.colorText(arg.y,1,32)}]`;
...
Util.toString = (obj, opts = {}, indent = '') => {
  const { quote = '"', multiline = true, color = true, spacing = ' ' } = opts;
  const sep = multiline ? (space = false) => '\n' + indent + (space ? '  ' : '') : (space = false) => space ? spacing : '';
  if(Util.isArray(obj)) {
    let s = Util.colorText('[', 1,36);
    for(let i = 0; i < obj.length; i++) {
      s += i > 0 ? Util.colorText(',',1,36) : '';
      s += sep(true);
  ...
  for(let key in obj) {
    ...
    s += sep(true);
    s += `${Util.colorText(key,1,33)}${Util.colorText(':',1,36)}`+spacing;
    /*if(Util.isArray(value)) s+= Util.toString(value);
      else*/ if(typeof value == 'object') s += Util.toString(value, opts, indent + '  ');
    else if(typeof value == 'string') s += Util.colorText(`${quote}${value}${quote}`, 1, 36);
    else if(typeof value == 'number') s += Util.colorText(value , 1, 32);
    else s += value;
    i++;
  }
  return s + sep(true) + Util.ansiCode(1,36)+'}';
};
...
Util.getMethodNames = function(obj, recursive = false) {
  return Util.members(obj, recursive, item => typeof obj[item] === 'function' && item != 'constructor');
};
...
Util.ansiCode = Util.isBrowser()
  ? () => ''
  : (...args) => `\u001b[${[...args].join(';')}m`;
  
Util.colorText = Util.isBrowser()
  ? (text, ...color) => (color.indexOf(1) != -1 ? `${text}` : text)
  : (text, ...color) => Util.ansiCode(...color) + text + Util.ansiCode(0);