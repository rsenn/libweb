...
Util.stack.prototype = Object.assign(Util.stack.prototype, {
  toString(opts = {}) {
    ...
    let a = [...this].map(frame =>
      Util.stackFrame.prototype.toString.call(frame, colors, { columnWidths, stripUrl })
    );
    let s = a.join('\n');
    return s + '\n';
  }, [Symbol.toStringTag]() {
...
Util.lazyProperty(Util, 'now', () => {
  ...
  if(performanceNow) {
    ...
    performanceNow = Util.bind(performanceNow, performance);
  }
...
Util.instrument = (fn,
  log = (duration, name, args, ret) =>
    console.log(`function '${name}' [${args.join(', ')}] {= ${ret}} timing:`, duration.toString())
) =>
  Util.isAsync(fn)
    ? async function(...args) {
        ...
        Util(g);
        log(Util.hrtime(start), Util.functionName(fn) || '<anonymous>', args, ret);
        return ret;
      }
    : function(...args) {
        const start = Util.hrtime();
