...
export function RGBA(...args) {
  ...
  if(args.length == 1 && Util.isArray(args[0]) && args[0].length >= 3)
    args = args[0];
  //Util.log('RGBA(', args, ')');
  ...
  if(args.length >= 3) {
    ...
    if(typeof arg === 'number') {
      Object.assign(ret,
        RGBA.decode[args[1] !== undefined ? args[1] : RGBA.order.ABGR](arg)
      );
...
export const isRGBA = obj =>
  RGBA.properties.every(prop => obj.hasOwnProperty(prop));
...
RGBA.encode = [
  /*RGBA:*/ ({ r, g, b, a }) =>
    [r, g, b, a].map(n => ('00' + (n & 0xff).toString(16)).slice(-2)).join(''),
  /*BGRA:*/ ({ r, g, b, a }) =>
    [b, g, r, a].map(n => ('00' + (n & 0xff).toString(16)).slice(-2)).join(''),
  /*ARGB:*/ ({ r, g, b, a }) =>
    [a, r, g, b].map(n => ('00' + (n & 0xff).toString(16)).slice(-2)).join(''),
  /*ABGR:*/ ({ r, g, b, a }) =>
    [a, b, g, r].map(n => ('00' + (n & 0xff).toString(16)).slice(-2)).join('')
];
RGBA.fmt = [
  ({ r, g, b, a }) => [r, g, b, a],
  ({ b, g, r, a }) => [b, g, r, a],
...
RGBA.prototype.compareTo = function(other) {
  let d =
    RGBA.prototype.binaryValue.call(other) -
    RGBA.prototype.binaryValue.call(this);
...
RGBA.prototype.hex = function(opts = {}) {
  ...
  return (prefix +
    ('0000000000' + n.toString(16))
      .slice(-8)
      .slice(0, a === 255 || a === undefined ? 6 : 8)
  );
};
...
RGBA.prototype.toCSS = function(fmt = num => +num.toFixed(3)) {
  ...
  if(a === undefined || a == 255)
    return 'rgb(' + fmt(r) + sep + fmt(g) + sep + fmt(b) + ')';
  return ('rgba(' +
    fmt(r) +
    sep +
    fmt(g) +
...
RGBA.prototype.toSource = function(sep = ',') {
  let a = this.a;
  if(a === undefined)
    return 'new RGBA(' + this.r + sep + this.g + sep + this.b + ')';
  let s =
    'new RGBA(' + toHex(this.r) + sep + toHex(this.g) + sep + toHex(this.b);
...
RGBA.blend = (a, b, o = 0.5) => {
  a = new RGBA(a);
  b = new RGBA(b);
  return new RGBA(Math.round(a.r * (1 - o) + b.r * o),
...
RGBA.prototype.toHSLA = function() {
  ...
  return new (Object.isFrozen(this) ? ImmutableHSLA : HSLA)(Math.round(h),
...
RGBA.prototype.blackwhite = function(a = this.a) {
  return this.luminanace() >= 0.558
    ? new RGBA(255, 255, 255, a)
    : new RGBA(0, 0, 0, a);
};
...
RGBA.prototype.toAnsi256 = function(background = false) {
  ...
  const toString = (background = false) =>
    `\x1b[${background ? 48 : 38};5;${value}m`;
...
RGBA.prototype[Symbol.for('nodejs.util.inspect.custom')] = function() {
  ...
  return (`\x1b[1;31mRGBA\x1b[1;36m` +
    `(${ret})`.padEnd(18, ' ') +
    ` ${color}    \x1b[0m`
  );
};
