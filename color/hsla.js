...
import { ImmutableRGBA, RGBA } from './rgba.js';
/**
...
HSLA.prototype.transform = function(t = (v, i) => v) {
  const cl = clamp(0, 100),
    cl1 = clamp(0, 1);
  const [h, s, l, a] = [...this].map(t);

  return new HSLA(h % 360, cl(s), cl(l), cl1(a));
};
...
HSLA.prototype.toAnsi = function() {
  const rgba = HSLA.prototype.toRGBA.call(this);
  return RGBA.prototype.toAnsi.call(rgba);
};
...
const InspectSymbol = Symbol.inspect ?? Symbol.for('quickjs.inspect.custom') ?? Symbol.for('nodejs.util.inspect.custom');

function toHex(n) {
  return '0x' + ('00' + (+n).toString(16)).slice(-2);
}

HSLA.prototype[InspectSymbol] = function(options = {}) {
  const { h, s, l, a } = this;
  const arr = a !== undefined && a != 255 ? [h, s, l, a] : [h, s, l];
  const ret = arr.map((n, i) => (i > 3 ? n : Math.round(n))).join(',');
  const color = this.toAnsi(true).replace(/\b38\b/g, '48');

  return `\x1b[1;33mHSL${arr.length == 4 ? 'A' : ' '}\x1b[1;36m` + `(${ret})`.padEnd(15, ' ') + ` ${color}    \x1b[0m`;
};
